		...
		try{
			...
			try{
				if (fileName.length() == 0){
					throw new FileNameMissingException("File name is missing!");
				}
			...
			try{
				if (makelLine.split(":")[0].equals("Make") && makelLine.split(":").length != 2)
					throw new FieldMissingException("'Make' field is missing!");
				else
			...
			try{
				if (modelLine.split(":")[0].equals("Model") && modelLine.split(":").length != 2)
					throw new FieldMissingException("'Model' field is missing! ");
				else
			...
			try{
				if (basePriceLine.split(":")[0].equals("Base Price") && basePriceLine.split(":").length != 2)
					throw new FieldMissingException("'Base Price' field is missing!");
				else
					...
					for (int optIndex=0; optIndex<optionSize; optIndex++){
						...
						try{
							if (optSplit.length != 2)
								throw new NoSuchOptionException("Missing option data! " +
